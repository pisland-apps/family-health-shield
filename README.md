...
- v45(2026-09-22)v44 之后做的一次文档/清理复审，没有新功能：①v41 那个全局右下角"打印当前页"按钮（`printCurrentPage()`）连同它专属的 `@media print` 样式块，在 v41→v44 的重构过程中已经被移除，但一直没在 changelog 里记一笔——这次确认这是**计划内的功能退役**：9 个结构化报表（Emergency Card / Health Summary / 疫苗 / 用药 / 检验 / 血压 / 生长曲线 / 年度报告 / 保单结单）已经覆盖了原来"打印当前页"想解决的需求，而且信息更完整、排版更统一，所以不再需要一个笼统地把当前屏幕截下来打印的按钮；孤立无引用的那段 `@media print` CSS 已删除。②修正了 4 处还停留在"9 个打印弹窗、各自带一份更宽松 CSP"这个旧描述上的设计说明/安全注释（`index.html` 顶部 CSP 设计说明的第 2、3 点、CSS 工具类那段注释、"In-app print reports"那段注释，以及 `app.js` 里 `sanitizeVitals()` 上方的注释）——这套弹窗架构在 v41-v44 已经整体退役，这次把文档改到跟实际架构一致，避免以后审计或改动时被过时描述误导。CSS 改动触及了 `<style>` 块，`regen-style-hash.py` 重新生成并同步到 `index.html`/`_headers` 两处。`APP_VERSION`/`CACHE_VERSION` 已同步改成 v45。
- v46(2026-09-23)修复 v45 发布后版本号角标（右下角 `#versionBadge`）不再显示的问题。根因是 v45 那次改动更新的 CSS 工具类注释里出现了字面意义上的 `*/`（原句列出了 `.pr-*/.pe-*/.pir-*/.rpt-*` 这几个报告类前缀，用 `/` 分隔时和结尾的 `*` 连在一起，凑巧组成了 CSS 注释结束符），把整段注释提前截断，导致注释末尾到真正结束符之间那段文字被当成"CSS 代码"解析成一堆非法内容，紧跟在后面的 `.s-00642396`（版本角标那条规则）就在这段解析恢复期间被浏览器整体丢弃——`regen-style-hash.py` 算出来的哈希照样是对的（哈希只管字节内容，不管内容能不能被正确解析成 CSS），CSP 也完全没有报错，纯粹是"某一条规则在解析过程中静默消失"，比 v42 那次（哈希本身算错）更隐蔽。用 Playwright 逐步二分定位到具体是哪段注释导致的，确认后把这句话改写成不含字面 `*/` 的说法（`.pr-`、`.pe-`、`.pir-`、`.rpt-` 之间用逗号分隔，不再用 `/` 加 `*` 的写法）。顺带修了两处在 v45 之前就一直遗留、被 CSP 静默拦截的内联 `style="..."` 属性（`btnConflicts` 按钮的 `display:none`、冲突处理弹窗的 `max-width:640px`），改成了 `.s-hidden`（已有的通用类，同时把 `app.js` 里对应的 `.style.display = ...` 改成了 `classList.toggle('s-hidden', ...)`，否则会跟这条类规则互相打架）和新加的 `.s-conflict-modal` 两个 class。CSS 改动触及了 `<style>` 块，`regen-style-hash.py` 重新生成并同步到 `index.html`/`_headers` 两处。`APP_VERSION`/`CACHE_VERSION` 已同步改成 v46。
- v47(2026-09-23)**style-src 的哈希白名单方案整体废弃**，改成跟 script-src（见本文件前面"应用逻辑现在是独立的 `app.js` 文件"那段）完全一样的思路：把 `<style>` 块整体挪到外部 `styles.css` 文件，`index.html` 里用 `<link rel="stylesheet" href="styles.css">` 引入，`index.html`/`_headers` 两处的 style-src 都直接写 `'self'`，不再带任何哈希。动机是这套哈希方案在生产里已经三次让界面"裸奔"或部分失效——2026-08-10 上线时完全没留哈希、v42 注释里的字面 `<style>` 截错提取位置、v46 注释里的字面 `*/` 截断注释导致单条规则静默丢失——三次的共同点是"只要 `<style>` 块里的注释文字凑巧撞上几个特定字符组合，就会在哈希照算不误、CSP 毫无报错的情况下让部分或全部样式失效"，属于结构性问题，不是靠"写注释时小心"或"多测试"能彻底避免的。改成 `'self'` + 外部文件后，浏览器校验的是"是不是同源的这个文件"而不是"内容有没有一字不差"，普通编辑（包括改注释）不再可能触发这类问题；缺点是失去了"篡改 `<style>` 内容会被 CSP 拦截"这层防护，但风险可控——原理跟 script-src 当初改用 `'self'` 时的取舍一致（见 `index.html` 顶部设计说明第 1 点），且 `index.html`（承载 `<link>` 标签本身）和 `app.js` 都已经不受任何字节级完整性保护，`styles.css` 单独留着这层保护意义有限。`regen-style-hash.py` 已删除；`index.html` 里原来那段 ⚠️ 哈希维护提醒和 README 前面"改 `<style>` 块之前必读"那条提醒都改成了标注"仅供历史参考"，没有整段删掉，保留下来是为了让以后翻 changelog 的人能看懂当初为什么走了哈希这条路、又是怎么踩了三次坑。`service-worker.js` 的预缓存列表（`APP_SHELL`）已加入 `./styles.css`；`_headers` 里新增了 `/*.css` 的显式 `Content-Type: text/css; charset=utf-8`（跟原有的 `/*.mjs` 规则一个思路）——这条不是可有可无的装饰，因为 `_headers` 里本来就开着 `X-Content-Type-Options: nosniff`，如果 Cloudflare Pages 对 `.css` 的自动 MIME 类型判断跟浏览器期望的不一致，`nosniff` 会导致浏览器直接拒绝把响应当样式表用，所以显式声明更稳妥。`APP_VERSION`/`CACHE_VERSION` 已同步改成 v47。**部署后待验证**（还没有实际部署验证过，下次发布时务必检查）：① `styles.css` 请求的响应头 `Content-Type` 确实是 `text/css`；② `index.html` 里 `<meta>` CSP 和 `_headers` 里的 CSP 两处 `style-src` 完全一致（都是不带哈希的 `'self'`）；③ Service Worker 强制刷新后离线模式下样式仍然正常加载（确认 `styles.css` 真的进了预缓存，不是网络请求兜底成功掩盖了缓存没生效的问题）。
